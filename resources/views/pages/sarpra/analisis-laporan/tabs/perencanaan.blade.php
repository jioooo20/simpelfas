<div class="flex flex-col lg:flex-row gap-6">
    ...
    <div class="w-full lg:w-1/2 bg-white p-6 rounded-lg shadow relative">
        ...
        {{-- Daftar Rekomendasi Dinamis --}}
        <div class="space-y-3 h-80 overflow-y-auto pr-2 custom-scrollbar">

            @forelse ($rekomendasiMaintenance as $rekomendasi)
                <a href="#" class="block bg-white border border-gray-200 p-4 rounded-lg hover:shadow-lg hover:border-blue-500 transition-all duration-200 group">
                    <div class="flex justify-between items-center">
                        <div class="flex-1 min-w-0 pr-4">
                            <div class="flex items-center">
                                <p class="text-sm font-semibold text-gray-900 truncate" title="{{ $rekomendasi['title'] }}">
                                    {{ $rekomendasi['title'] }}
                                </p>
                            </div>
                            ...
                            <div class="text-xs text-gray-600 mt-3 pt-3 border-t border-gray-200">
                                <span class="font-medium {{ $statusColors[$rekomendasi['status_color']]['text'] ?? 'text-gray-600' }}">{{ $rekomendasi['reports'] }} laporan</span>
                                <span class="mx-1.5">&bull;</span>
                                <span class="font-medium {{ $statusColors[$rekomendasi['status_color']]['text'] ?? 'text-gray-600' }}">{{ number_format($rekomendasi['satisfaction'], 2) }}/5 kepuasan</span>
                                <span class="mx-1.5">&bull;</span>
                                <span class="font-medium {{ $statusColors[$rekomendasi['status_color']]['text'] ?? 'text-gray-600' }}">{{ $rekomendasi['interval'] }} hari interval</span>
                            </div>
                        </div>
                        ...
                        <span class="status-badge inline-flex items-center px-3 py-1 rounded-full text-xs font-medium
                              ...
                              data-description="@switch($rekomendasi['status'])
                                     ...
                                     @endswitch"
                        >
                            {{-- Ikon dinamis berdasarkan status_color --}}
                            @if ($rekomendasi['status_color'] == 'red')
                                <svg class="w-3.5 h-3.5 mr-1.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.21 3.03-1.742 3.03H4.42c-1.532 0-2.492-1.696-1.742-3.03l5.58-9.92zM10 13a1 1 0 110-2 1 1 0 010 2zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" /></svg>
                            @elseif ($rekomendasi['status_color'] == 'yellow')
                                <svg class="w-3.5 h-3.5 mr-1.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.414-1.415L11 9.586V6z" clip-rule="evenodd" /></svg>
                            @else
                                <svg class="w-3.5 h-3.5 mr-1.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" /></svg>
                            @endif

                            {{-- Tampilkan label aksi yang sudah diolah --}}
            ...
            @empty
                <div class="h-full flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-200 text-center px-6 bg-green-50">
                    <div class="flex-shrink-0">
                    ...
                    <div class="mt-2">
                        ...
                        <p class="mt-1 text-sm text-green-700">Saat ini tidak ada rekomendasi maintenance yang perlu diprioritaskan.</p>
                    </div>
                </div>
            @endforelse
        </div>

        <div id="status-popover" class="hidden absolute z-20 w-64 text-sm rounded-lg shadow-xl transition-opacity duration-200 opacity-0">
