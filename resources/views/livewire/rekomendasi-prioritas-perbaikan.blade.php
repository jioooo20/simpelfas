<div>
    ...
    <div class="flex justify-between items-center mb-6 p-4 bg-gray-50 rounded-lg border">
        ...
        <div>
            <button wire:click="olahDss" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-lg shadow-lg transition duration-200">
                OLAH DSS
            </button>
        </div>
    </div>
    ...
    <div class="mb-6">
        <div class="border-b border-gray-200">
            <nav class="-mb-px flex space-x-8">
                ...
                <button
                    wire:click="switchTab('mahasiswa')"
                ...
                <tbody>
                    @forelse ($laporanData as $index => $item)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="text-center">
                                <span class="px-2 py-1 rounded-md text-sm {{ $item->c1 > 0 ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-500' }}">
                                    {{ $item->c1 > 0 ? number_format($item->c1, 2) : '-' }}
                                </span>
                            </td>
                            <td class="text-center">
                                <span class="px-2 py-1 rounded-md text-sm {{ $item->c2 > 0 ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-500' }}">
                                    {{ $item->c2 > 0 ? number_format($item->c2, 2) : '-' }}
                                </span>
                            </td>
                            <td class="text-center">
                                <span class="px-2 py-1 rounded-md text-sm {{ $item->c3 > 0 ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-500' }}">
                                    {{ $item->c3 > 0 ? number_format($item->c3, 2) : '-' }}
                                </span>
                            </td>
                            <td class="text-center">
                                <span class="px-2 py-1 rounded-md text-sm {{ $item->c4 > 0 ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-500' }}">
                    ...
                    <div class="bg-gray-50 rounded-lg p-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-900">{{ $selectedLaporan->fasilitas->fasilitas_kode }}</p>
                            </div>
                            <div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            @foreach ([1 => 'C1', 2 => 'C2', 3 => 'C3', 4 => 'C4'] as $kriteriaId => $kode)
                                <div class="text-center">
                                    <div class="text-2xl font-bold {{ isset($selectedLaporan->avg_scores[$kriteriaId]) ? 'text-green-600' : 'text-gray-400' }}">
                            ...
                            <table class="min-w-full">
                                <thead class="bg-gray-100">
                                    <tr>
                                        ...
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Pelapor</th>
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    @foreach ($selectedLaporan->reports as $report)
                                        <tr>
                                            <td class="px-3 py-2 text-sm text-gray-900">{{ $report->pelaporan_kode }}</td>
    ...
    @if ($showDssResultModal && $dssResults)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-white rounded-lg shadow-xl max-w-6xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                <!-- Header Modal -->
                <div class="flex items-center justify-between p-6 border-b border-gray-200">
                    <h3 class="text-xl font-semibold text-gray-900">Hasil Pengolahan DSS</h3>
                    <button wire:click="$set('showDssResultModal', false)" class="text-gray-400 hover:text-gray-600">
                        <i class="bi bi-x-lg text-xl"></i>
                    </button>
                </div>
                    ...
                    <div class="bg-blue-50 rounded-lg p-4">
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-100">
                                    <tr>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Kode Fasilitas</th>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Gedung</th>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Lantai</th>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Ruangan</th>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Nama Barang</th>
                                        <th class="px-3 py-2 text-center text-xs font-medium text-gray-500 uppercase">Skor</th>
                                        <th class="px-3 py-2 text-center text-xs font-medium text-gray-500 uppercase">Ranking</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-200 bg-white">
                                    ...
                                    @php
                                        $facilityInfo = DB::table('t_fasilitas as f')
                                            ...
                                            ->select('f.fasilitas_kode', 'b.barang_nama', 'g.gedung_nama', 'l.lantai_nama', 'r.ruang_nama')
                                            ->first();
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-3 py-2 text-center text-sm text-gray-900">{{ number_format($score, 4) }}</td>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-100">
                                    <tr>
                                        ...
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Ruangan</th>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Nama Barang</th>
                                        <th class="px-3 py-2 text-center text-xs font-medium text-gray-500 uppercase">Skor Borda</th>
                                        <th class="px-3 py-2 text-center text-xs font-medium text-gray-500 uppercase">Ranking Final</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-200 bg-white">
                                    ...
                                    <tr class="{{ $loop->first ? 'bg-yellow-100' : '' }}">
                                        <td class="px-3 py-2 text-sm {{ $loop->first ? 'text-yellow-900' : 'text-gray-900' }}">
                                            {{ $facilityCode }}
                                            @if($loop->first)
                                                <span class="text-xs text-yellow-600 ml-1">(Prioritas Tertinggi)</span>
                                            @endif
                                        </td>
                                        <td class="px-3 py-2 text-sm {{ $loop->first ? 'text-yellow-900' : 'text-gray-900' }}">
                                            {{ $facilityInfo->gedung_nama ?? '-' }}
                                        ...
                                        <td class="px-3 py-2 text-sm {{ $loop->first ? 'text-yellow-900' : 'text-gray-900' }}">
                                            {{ $facilityInfo->barang_nama ?? 'Unknown' }}
                                            <span class="{{ $loop->first ? 'text-yellow-600' : 'text-black' }} font-medium">{{ $lastTwoDigits }}</span>
                                        </td>
                                        <td class="px-3 py-2 text-center text-sm {{ $loop->first ? 'text-yellow-900 font-medium' : 'text-gray-900' }}">
                                            {{ number_format($score, 2) }}
                                        </td>
                                        <td class="px-3 py-2 text-center">
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $loop->first ? 'bg-yellow-200 text-yellow-800' : 'bg-gray-100 text-gray-800' }}">
                ...
                <!-- Footer Modal -->
                <div class="flex justify-end p-6 border-t border-gray-200">
                    <button wire:click="$set('showDssResultModal', false)" class="btn btn-sm btn-ghost">
                        Tutup
                    </button>
                </div>
            </div>
