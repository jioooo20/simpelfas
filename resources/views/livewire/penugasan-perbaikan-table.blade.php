                ...
                @forelse ($perbaikanData as $index => $pelaporan)
                    @php
                        ...
                        if ($pelaporan->perbaikan) {
                            $assignedTechnicians = $pelaporan->perbaikan->perbaikanPetugas
                                ->pluck('user.nama')
                                ->join(', ');
                            $currentStatus = $pelaporan->perbaikan->statusPerbaikan->perbaikan_status ?? 'Menunggu';
                    ...
                    <tr class="hover" wire:key="row-{{ $pelaporan->pelaporan_id }}">
                        ...
                        <td class="text-center">
                            @php
                                // Ambil status dari t_status_perbaikan jika ada, jika tidak fallback ke statusPelaporan
                                $status = '-';
                                $badgeClass = 'badge-ghost';
                                if ($pelaporan->perbaikan && $pelaporan->perbaikan->statusPerbaikan) {
                                    $status = $pelaporan->perbaikan->statusPerbaikan->perbaikan_status;
                                }
                                if ($status === 'Diproses') {
                        ...
                        <td class="text-center">
                            @php
                                // Ambil status pelaporan terbaru (t_status_laporan)
                                $statusPelaporan = $pelaporan->statusPelaporan->sortByDesc('created_at')->first();
                            @endphp
                            <div class="text-sm">
                                {{ $statusPelaporan ? $statusPelaporan->created_at->format('d/m/Y') : ($pelaporan->created_at ? $pelaporan->created_at->format('d/m/Y') : '-') }}
                            </div>
                            <div class="text-xs text-gray-500">
                                {{ $statusPelaporan ? $statusPelaporan->created_at->format('H:i') : ($pelaporan->created_at ? $pelaporan->created_at->format('H:i') : '-') }}
                            </div>
                        </td>
                        <td class="text-center">
                    ...
                    <div class="card-body">
                        ...
                        <div class="space-y-3 mt-4">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span>{{ $selectedPerbaikan->perbaikan && $selectedPerbaikan->perbaikan->statusPerbaikan ? $selectedPerbaikan->perbaikan->statusPerbaikan->perbaikan_status : 'Menunggu Penugasan' }}</span>
                            </div>
                            <div class="">
